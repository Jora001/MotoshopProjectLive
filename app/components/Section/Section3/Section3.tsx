...
const Section3 = () => {
  ...
  const CARD_WIDTH = 386; // 350 + 36 gap
  const INTERVAL = 3000;
  const visibleSlides = 3; // Քանի քարտ ցույց է տալիս միանգամից
  const totalSlides = cards.length - visibleSlides + 1; // Radio indicators համահունչ slides
              ...
              <div className="relative w-[350px] h-[250px] bg-white">
                <div className="absolute top-[45px] left-0 flex flex-col gap-[6px] z-10">
                  {card.badges.map((badge) => (
                    <div
                      key={badge}
                      className={`min-w-[80px] h-[22px] px-[10px] py-[2px]
                      text-[12px] font-semibold flex items-center justify-center
                      whitespace-nowrap rounded-[1px]
